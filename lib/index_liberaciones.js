...
var arrayVariables = [["nivl1Sistms","entry.525199900","Nivel 1 Sistemas","TEXTO"],
					  ...
					  ["fchIniInst02","entry.611655675_month","Mes de Fecha Inicio de Instalaci\u00f3n","TEXTO"],
					  ["fchIniInst03","entry.611655675_year","Anho de Fecha Inicio de Instalaci\u00f3n","TEXTO"],
...
function handleQueryResponse(response) {
		...
		formatter_link.format(data, [10, 4, 12, 6, 5, 17, 18, 16, 9]);
		
		var view = new google.visualization.DataView(data);
		view.setColumns([10, 4, 12, 6, 5, 17, 18, 16]);
		...
		google.visualization.events.addListener(table, 'ready',
				function(event) {
						$(".inline").colorbox({inline:true, width:"90%", height:"90%", onclick: function() {alert("xx")}});
	...
	for (var row = 0; row < data.getNumberOfRows(); row++) {
		if (elemento.text == data.getValue(row, column10)) {
			...
			for (var col = 1; col < data.getNumberOfColumns() - 1; col++) {
				...
				if (col < data.getNumberOfColumns() - 2){
...
function regstrNuevo(){
	window.location.href = 'https://docs.google.com/a/bbva.com/forms/d/1iiWoKJC4ClhIubn4G36vJZxh0D17uanzCfzHw27g_ZM/viewform?usp=send_form'
}
...
function enviaDatosAForm(){
	...
	if(enviaDatos){
		...
		//submitFinal = submitFinal.replace("á","\u00e1")
